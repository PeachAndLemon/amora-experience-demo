...
import { ChevronLeft, Check, PartyPopper, Palette, MessageCircle, Flame, Mountain } from 'lucide-react';
import { WantMoreOf } from '@/types/amora';

const wantMoreOptions: {id: WantMoreOf;label: string;icon: React.ReactNode;}[] = [
{ id: 'play-laughter', label: 'Play & laughter', icon: <PartyPopper className="w-5 h-5" /> },
{ id: 'creating-together', label: 'Creating together', icon: <Palette className="w-5 h-5" /> },
{ id: 'presence-deep-talks', label: 'Presence & deep talks', icon: <MessageCircle className="w-5 h-5" /> },
{ id: 'romance-desire', label: 'Romance & desire', icon: <Flame className="w-5 h-5" /> },
{ id: 'adventure-challenge', label: 'Adventure & challenge', icon: <Mountain className="w-5 h-5" /> }];


interface WantMoreScreenProps {
  onComplete: (selected: WantMoreOf[]) => void;
  onBack: () => void;
  initialValue?: WantMoreOf[];
}

export function WantMoreScreen({ onComplete, onBack, initialValue = [] }: WantMoreScreenProps) {
  const [selected, setSelected] = useState<WantMoreOf[]>(initialValue);
  const toggleOption = (id: WantMoreOf) => {
    setSelected((prev) =>
    ...
    <div className="min-h-screen flex flex-col bg-[#91032e]">
      <div className="flex items-center justify-between p-6 safe-top">
        ...
        <div className="flex gap-1">
          {[1, 2, 3, 4, 5, 6].map((i) => <div key={i} className="w-6 h-1 rounded-full bg-gold/80" />)}
          {[7, 8, 9, 10].map((i) => <div key={i} className="w-6 h-1 rounded-full bg-cream/20" />)}
        </div>
        <div className="w-10" />
      </div>
      ...
      <div className="flex-1 px-6 pb-6 overflow-y-auto">
        <div className="text-center mb-6">
          <h1 className="font-serif text-2xl md:text-3xl font-semibold text-cream mb-2">What do you want more of in the next 60 days?</h1>
